...
This experimental adapter refreshes the development PG 1159 model. The
PG 1424+535 preset has been qualified from a cold start against the legacy
spectrum; this is not yet a validated temperature, gravity, or abundance
grid. An unfinished calculation
returns an explicitly unverified atmosphere. `run_model(...,
require_convergence=True)` rejects that result.

```python
from wd_spectra import PG1159Config, ModelData, run_model
...
The He/C/O population iteration uses a shallow six-state acceleration history
away from the root and switches to the plain physical map once the raw defect
is within three times the requested target. A separate undamped rate-map
...
A public call always starts at the requested star's own continuum atmosphere.
...
The result is then given a dimensionless non-gray temperature shape measured
from the accepted legacy PG 1424+535 He/C/O structure and scaled to the
requested Teff. The 32-point shape is built into the package; no atmosphere or
population checkpoint is discovered or loaded at runtime. The shared solver
still re-evaluates every material, transfer, and balance equation. A fixed
1.0885 normalization accounts for the migrated opacity set's measured
bolometric flux scale on the legacy column-mass grid.
...
PG 1159 certification uses the quantities that constrain the emergent
spectrum: the emergent bolometric surface flux and local heating over
`1e-6 <= tau_Ross < 10` must agree within 0.75% and 1.5%, respectively. The maximum internal
flux error for both `tau_Ross < 10` and the complete grid through
`tau_Ross = 100` remains in the certificate as a non-gating diagnostic.
Scattering-source closure must be below `1e-6`, the
lower-boundary escape bound below 0.75%, and hydrostatic and electron closures
...
the development solver as well. This reproduces the physical gates used by
the legacy PG 1159 solver while making its waiver and the internal flux
diagnostics explicit. The declared residual window and tolerances are recorded
...
5500 Angstrom. These measured spectral changes, rather than an unreachable
inner tolerance, set the supported stopping point. Iteration-budget exhaustion
and any failed required certificate check remain explicitly unconverged.
...
Use a `ModelData` root containing `cache/` (or the development `.cache/`):
...
The helium profiles, Stout archive contents, and Verner data are already
provided by the OpenWD data installation. The additional CCC, TLUSTY, TMAD,
SIROCCO, and CHIANTI files must be supplied separately; the adapter reports
missing paths before starting. It records the selected atomic-file hashes
and actual model-atom counts in the output.
